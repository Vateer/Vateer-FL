...
class Handler(BaseHandler):
    ...
    # 加载MNIST训练数据
    def load_data(self):
        if not os.path.exists(self.cfg["base_path"] + "/data/fmnist"):
            os.makedirs(self.cfg["base_path"] + "/data/fmnist")
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        data_train = datasets.FashionMNIST(root = self.cfg["base_path"] + "/data/fmnist",
                                    # transform=transform,
                                    train = True, transform=transform,
                                    download = True)
        # data_test = datasets.FashionMNIST(root = self.cfg["base_path"] + "/data/fmnist",
        #                         #    transform = transform,
    ...
    def save_data_to_folders(self, data_splits, num_clients):
        save_dir = self.cfg["base_path"] + '/data/fmnist/client_data/{}'.format(str(num_clients))
    ...
    def load_data_from_folders(self, client_id, num_clients):
        data_dir = os.path.join(self.cfg["base_path"] + '/data/fmnist/client_data/{}/client_{}'.format(str(num_clients), str(client_id)))
...
class DataLoader:
    def __init__(self, cfg, dev_id, batch_size = None):
        self.cfg = cfg
        self.data_dir = os.path.join(cfg["base_path"]+r"/data/fmnist/client_data/{}".format(str(cfg["num_clients"])),"client_{}".format(str(dev_id)))
